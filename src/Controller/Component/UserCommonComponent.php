...
class UserCommonComponent extends Component {
    ...
    public function checkAuthorizeUser($toId, $dbId, $postedRoles = []) {
        ...
        $loggedUserRoles = $this->getUserDatabasesRoles($authuserId, $dbId);//get roles of logged in user on db
        $toUserRoles = $this->getUserDatabasesRoles($toId, $dbId);  //get roles other   user on db
        $returnValue = '';
    ...
    /*
      ...
      @roleValue will be type of role // like ADMIN
    ...
    public function checkAdminRoleAccess($roleValue, $loggedUserRoles = [], $toUserRoles = [], $postedRoles = []) {
        ...
        return 'NA'; //when not a DE or TEMPLATE user 
    }
    ...
    public function saveUserDetails($inputArray = array(), $dbId = null) {
        ...
        if (!isset($inputArray['dbId']))
            $inputArray['dbId'] = $dbId;
